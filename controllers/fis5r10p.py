...
#   * inputs : error theta y error
#   obtener omega


def factor(valor, minimum, maximum):
    assert minimum < maximum
    return minimum + (maximum - minimum) * valor
...
def fis_opt(e_theta, error, params, grafica=False):
    ...
    e_theta_med_neg = fuzz.trimf(x_e_theta, [-d-e, -d, -d+e])
    e_theta_lo = fuzz.trimf(x_e_theta, [-a, 0, a])
    e_theta_med_pos = fuzz.trimf(x_e_theta, [d-e, d, d+e])
    e_theta_hi_pos = fuzz.trapmf(x_e_theta, [b-c, b, 5, 50])
    ...
    e_theta_level_hi_neg = fuzz.interp_membership(x_e_theta, e_theta_hi_neg, e_theta)
    e_theta_level_med_neg = fuzz.interp_membership(x_e_theta, e_theta_med_neg, e_theta)
    e_theta_level_lo = fuzz.interp_membership(x_e_theta, e_theta_lo, e_theta)
    e_theta_level_med_pos = fuzz.interp_membership(x_e_theta, e_theta_med_pos, e_theta)
    e_theta_level_hi_pos = fuzz.interp_membership(x_e_theta, e_theta_hi_pos, e_theta)
    ...
    if grafica:
        ...
        ax0.plot(x_e_theta, e_theta_hi_neg, 'b', linewidth=1.5, label='Alto negativo')
        ax0.plot(x_e_theta, e_theta_med_neg, 'm', linewidth=1.5, label='Medio negativo')
        ax0.plot(x_e_theta, e_theta_lo, 'g', linewidth=1.5, label='Bajo')
        ax0.plot(x_e_theta, e_theta_med_pos, 'k', linewidth=1.5, label='Medio positivo')
        ax0.plot(x_e_theta, e_theta_hi_pos, 'r', linewidth=1.5, label='Alto positivo')
    ...
    hi_neg_hi_neg = np.fmin(e_theta_level_hi_neg, error_level_hi_neg)
    hi_neg_med_neg = np.fmin(e_theta_level_hi_neg, error_level_med_neg)
    hi_neg_lo = np.fmin(e_theta_level_hi_neg, error_level_lo)
    hi_neg_med_pos = np.fmin(e_theta_level_hi_neg, error_level_med_pos)
    hi_neg_hi_pos = np.fmin(e_theta_level_hi_neg, error_level_hi_pos)

    med_neg_hi_neg = np.fmin(e_theta_level_med_neg, error_level_hi_neg)
    med_neg_med_neg = np.fmin(e_theta_level_med_neg, error_level_med_neg)
    med_neg_lo = np.fmin(e_theta_level_med_neg, error_level_lo)
    med_neg_med_pos = np.fmin(e_theta_level_med_neg, error_level_med_pos)
    med_neg_hi_pos = np.fmin(e_theta_level_med_neg, error_level_hi_pos)

    lo_hi_neg = np.fmin(e_theta_level_lo, error_level_hi_neg)
    lo_med_neg = np.fmin(e_theta_level_lo, error_level_med_neg)
    lo_lo = np.fmin(e_theta_level_lo, error_level_lo)
    lo_med_pos = np.fmin(e_theta_level_lo, error_level_med_pos)
    lo_hi_pos = np.fmin(e_theta_level_lo, error_level_hi_pos)

    med_pos_hi_neg = np.fmin(e_theta_level_med_pos, error_level_hi_neg)
    med_pos_med_neg = np.fmin(e_theta_level_med_pos, error_level_med_neg)
    med_pos_lo = np.fmin(e_theta_level_med_pos, error_level_lo)
    med_pos_med_pos = np.fmin(e_theta_level_med_pos, error_level_med_pos)
    med_pos_hi_pos = np.fmin(e_theta_level_med_pos, error_level_hi_pos)

    hi_pos_hi_neg = np.fmin(e_theta_level_hi_pos, error_level_hi_neg)
    hi_pos_med_neg = np.fmin(e_theta_level_hi_pos, error_level_med_neg)
    hi_pos_lo = np.fmin(e_theta_level_hi_pos, error_level_lo)
    hi_pos_med_pos = np.fmin(e_theta_level_hi_pos, error_level_med_pos)
    hi_pos_hi_pos = np.fmin(e_theta_level_hi_pos, error_level_hi_pos)
    ...
    aggregated = reduce(np.fmax, [activation_hi_neg, activation_med_neg,
                                  activation_lo, activation_med_pos, activation_hi_pos])
    # Calculate defuzzified result
    return fuzz.defuzz(x_omega, aggregated, 'centroid')  # return omega
