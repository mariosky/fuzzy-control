...
def Generador_de_poblaciones(strategies):
    ...
    for i, algorithm in enumerate(strategies):
        ...
        for llave in configBasica:  # para sacar un valor aleatorio del rango
            if (
                type(configBasica[llave]) == list
                and type(configBasica[llave][0]) == float
            ):
...
def combina_buffer(config, random=False, uniqueBuffer=False):
    ...
    while True:
        ...
        if mensaje_poblacion:
            ...
            if (
                num_total == config["num_poblaciones"] * config["num_cycles"]
            ):  # para salirse cuando llegue a 10 poblacioens
                ...
                print(
                    "best score:{0} best solution {1}".format(
            ...
            if random:
                poblacion["pop"] = poblacion["pop"][:-2] + [
                    popBuffer.random_choice() for i in range(2)
                ]
            else:
...
if __name__ == "__main__":
    ...
    if "CONFIG_REDIS" in os.environ:
        r = redis.StrictRedis(host=os.environ["CONFIG_REDIS"], port=6379, db=0)
        redis_ready = False
        print(os.environ["REDIS_HOST"])
        # intenta hasta que este listo el contenedor
        while not redis_ready:
